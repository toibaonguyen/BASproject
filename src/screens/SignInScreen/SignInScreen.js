...
const SignInScreen = () => {
  ...
  const onSignInPressed= async()=>{
    ...
    if(username.length==0||password.length==0){
      Alert.alert("Error!","Email and password inputs can't be blank",[{text:"OK"}])
      return

    }
    //dispatch(setisloading(true))
    await auth().signInWithEmailAndPassword(username,password)
    .then(()=>{console.warn("perfect")})
    .catch(error=>{
...
const styles=StyleSheet.create({logo:{width:'70%',height:"30%",maxHeight:300,maxWidth:200},root:{padding:20,alignItems:"center",flex:1}})
